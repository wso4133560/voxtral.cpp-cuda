...
struct voxtral_context {
    ...
    ggml_tensor * decoder_time_emb = nullptr; // [dec_dim] constant diffusion time embedding
    ...
    ggml_tensor  * attn_mask)  // [n_kv, n_tokens] or nullptr
{
    ...
    // This is critical for FP32->FP16 conversion in ggml_cpy
    k = ggml_reshape_2d(gctx, k, kv_dim, n_tokens);
    k = ggml_cont(gctx, k);

    // Ensure V is also contiguous for reliable FP32->FP16 conversion
    v = ggml_cont(gctx, v);
    ...
    bool                  store_logits)
{
    ...
    ggml_tensor * time_emb = ctx->decoder_time_emb; // [dec_dim] persistent backend tensor
    ...
    int32_t         * next_token_out)
{
    ...
    ggml_cgraph * gf = build_decoder_step_graph(
        ctx, gctx, position, audio_pos, logits_out != nullptr, &tok_t, &pos_t);
